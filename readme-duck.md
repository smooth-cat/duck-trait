# duck-trait — marker api (`#[duck]` / `#[_duck]` + `ducks!` / `#[duck_mod]`)

> The marker api generates the accessor traits in the scope where the struct is declared. For the
> field-based api (one crate-wide trait set declared in `_fields.rs`), see
> [readme.md](https://github.com/smooth-cat/duck-trait/blob/main/readme.md).
