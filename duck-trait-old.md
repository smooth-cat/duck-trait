# duck-trait — legacy field-marker flow（旧版字段标记流程）

The trait-first API (`#[props]` / `#[duck(_Show{..})]` / `#[ducky]`) is documented in
[readme.md](readme.md). This document describes the original struct-first flow, which is still
fully supported and can be mixed freely with the new one.

trait 优先的新 API（`#[props]` / `#[duck(_Show{..})]` / `#[ducky]`）见 [readme.md](readme.md)。
本文档描述最初的 struct 优先流程，目前仍然完全支持，并可与新流程自由混用。
...
- Trait visibility: `#[duck]` defaults to `pub(crate)`; `#[duck(pub)]` / `#[duck(pub = ..)]` set
  `pub` or restricted visibility; `#[_duck]` keeps the trait private (see "Generated trait
  visibility").
...
- trait 可见性：`#[duck]` 默认 `pub(crate)`；`#[duck(pub)]` / `#[duck(pub = ..)]` 可设为 `pub`
  或受限可见性；`#[_duck]` 保持私有（见「生成的 trait 可见性」）。
